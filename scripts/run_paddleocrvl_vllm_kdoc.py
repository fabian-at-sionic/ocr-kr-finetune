...
TITLE = "KOREAN PPStructureV3 + PaddleVL1.6 BENCHMARK"
...
def verify_gpu_paddle() -> dict[str, Any]:
    ...
    if cpu_version:
        raise RuntimeError("CPU package 'paddlepaddle' is installed. Refusing to run; install only 'paddlepaddle-gpu'.")
    if not gpu_version:
...
def pipeline_spec(args: argparse.Namespace, dataset_dir: Path) -> dict[str, Any]:
    return {
        "title": TITLE,
        "pipeline": {
            "class": "PaddleOCRVL",
    ...
    manifest = {
        ...
        "runtime": {
            **paddle_info,
            ...
            "flash_attn": package_version("flash-attn") or package_version("flash_attn"),
        },
...
def result_to_markdown(result: Any) -> str:
    ...
    if isinstance(markdown, dict):
        text = markdown.get("markdown_texts") or markdown.get("markdown") or markdown.get("content") or ""
    ...
    if isinstance(result, dict):
        ...
        if parsing:
            ...
            if parts:
                return "\n\n".join(parts)
        for key in ("markdown_texts", "rec_text", "text", "content"):
            if key in result:
...
def main() -> None:
    ...
    parser.add_argument("--candidate", default="paddleocrvl_1_6_0_9b_vllm")
    ...
    parser.add_argument("--use-layout-detection", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--use-chart-recognition", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--use-seal-recognition", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--use-ocr-for-image-block", action=argparse.BooleanOptionalAction, default=False)
    ...
    spec = pipeline_spec(args, dataset_dir)
    ...
    try:
        for index, pdf_path in enumerate(pending, start=1):
            start = time.time()
            md_path = out_dir / f"{pdf_path.stem}_pg1_repeat1.md"
            reporter.update(done=index - 1, phase="render", current_pdf=pdf_path.name)
            try:
                image = render_first_page(pdf_path, dpi=args.dpi)
                reporter.update(phase="ocr")
                results = list(
                    pipeline.predict(
                        np.asarray(image),
                        use_layout_detection=args.use_layout_detection,
